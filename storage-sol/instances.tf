...
resource "azurerm_linux_virtual_machine" "vm1" {
  ...
  custom_data = base64encode(templatefile("${path.module}/init/setup.tfpl", {
    html_url = azurerm_storage_blob.html.url
    png_url  = azurerm_storage_blob.png.url
  }))

  depends_on = [
    azurerm_storage_blob.html,
    azurerm_storage_blob.png
  ]
